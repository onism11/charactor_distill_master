...
S1 does not decide the default period. It records period candidates, source coverage, and alias risk for S0/S1 Gate to confirm or escalate before Rawcut.
...
Suspicious aliases, titles, relationship names, or unstable same-name references must go into `alias_gate` as suspicious or excluded candidates; never silently promote them to confirmed aliases.
