...
- `source_archive/character_rawcut.md` —— `character_slice`（人物事实）与 `plot_spine`（剧情/制度因果脊柱）**并列为一等输入**：canon 既要写角色本人做了什么，也要用 plot_spine 还原"为什么发生、角色为何无法单独解决"的制度/剧情因果，避免 canon 只围人物个人转。
- `check/Rawcut Evidence Brief.md`（含"剧情脊柱节点"块，用作因果导航）
- `source_archive/rawcut_slices_index.json`（固定产出的轻量机器索引）
...
Start from the character-nearest inputs: `Rawcut Evidence Brief.md`（先看"高价值人物证据"建人物事实，再看"剧情脊柱节点"建因果链），then the indexed target slices. Treat the full `source_archive/character_rawcut.md` and `source_archive/` as evidence storage, not default reading material.
...
世界 / 组织 / 制度 / 术语等设定材料**不另外生成 world.md**，按上文「世界设定材料归并约定」的路由归并进 canon.md。这是与 workdown benchmark 对齐的设计：workdown 没有独立 world.md，世界设定都内化进 canon。
...
- `check/Rawcut Evidence Brief.md`；`source_archive/rawcut_slices_index.json`（固定产出）
- `source_archive/character_rawcut.md` as evidence storage for targeted lookup
- all of `canon.md`
- confirmed alias list
- target period/version
- user exclusions
- `au_overrides.md`（仅当用户提供 AU 设定时才有此文件；Stage B 默认不读取，只在用户明确要求 AU 蒸馏时挂载）
